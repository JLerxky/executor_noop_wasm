...
`CITA-Cloud`中[executor微服务](https://github.com/cita-cloud/cita_cloud_proto/blob/master/protos/executor.proto)的实现。
...
1. `config` 微服务配置文件。
    ...
    其中`[executor_noop_wasm]`段为微服务的配置：
    ...
    * `domain` 节点的域名
    ...
    其中`[executor_noop_wasm.log_config]`段为微服务日志的配置：
    * `max_level` 日志等级
    * `filter` 日志过滤配置
